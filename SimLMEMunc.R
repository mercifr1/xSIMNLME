...
library(tidyverse)
df<-data.frame(x=1:100, y=rnorm(100, 10, 5), cat=rbernoulli(100, 0.2))

ggplot(df, aes(x, y))+
  geom_point()+
  theme_minimal()

#' Quoting and enquoting
var<-expr(mpg)
#see what happens with: var<-expr(mpg+1)
mtcars %>%
  group_by(cyl) %>%
  summarise(
...
#' From: https://speakerdeck.com/lionelhenry/programming-in-the-tidyverse
my_count_by<-function(mydf, ...){
  mydf %>%
    group_by(...) %>%
    summarise(n=n())
}
...
my_avg_by<-function(mydf, grp, yvar){
  mydf %>%
    group_by(!!enquo(grp)) %>%
    summarise(avg=mean(!!enquo(yvar), na.rm=T))
}
my_avg_by(df, cat, y)
